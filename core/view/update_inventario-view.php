...
if(isset($_POST)){
    ...
    && isset($_POST['cta_deb_ja']) && isset($_POST['cta_cred_aj']) && isset($_POST['cta_ivadif_vta']) && isset($_POST['cta_ivadif_comp'])){
        try{
            $inventario = LineaInventarioModel::GetById($_POST["id"]);
            ...
            if($inventario->Update()){
                Core::alert("Correcto","Linea inventario actualizada exitosamente","./?view=linea_inventario");
            }else{
                Core::alert("Error","La linea de inventario no se pudo registrar. Verifique que los datos estén correctos y el usuario no exista","./?view=linea_inventario");
            }
        }catch(Exception $ex){
